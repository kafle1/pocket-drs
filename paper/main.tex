...
reconstructed point. The present problem differs in three ways that drive the method: there is one
hand-held camera whose intrinsics are unknown, the object is a few pixels wide, and the uncertainty has to be
carried through a nonlinear fit and an extrapolation to a decision rather than stopping at a point.
...
not earned. Surveys of vision in sport~\cite{thomas2017sports} and of ball
...
to basketball shots in broadcast video and Chao et al.~\cite{chao2023basketball} do the same from a single
monocular view; Ponglertnapakorn and Suwajanakorn~\cite{ponglertnapakorn2025ball} learn to lift a 2-D ball
track to 3-D across several sports. All three take scale from gravity and none reports the uncertainty of
the result. Van Zandycke and De Vleeschouwer~\cite{vanzandycke2022ball3d} regress ball depth from its
...
the TrackNet family~\cite{huang2019tracknet,sun2020tracknetv2,chen2023tracknetv3} address the detection of small fast
balls;
Yan et al.~\cite{yan2008tennis} the association of candidates into tracks under clutter, which our
association stage follows in spirit.
...
standard~\cite{seber1989nonlinear}. Monocular depth networks~\cite{ranftl2022midas} recover relative depth of a static scene and
structure-from-motion~\cite{schonberger2016sfm} needs camera motion; neither uses the constraint that makes
...
returned as not out. Nearly every disagreement is between a decisive class and umpire's call in one
...
is a metre along the pitch, and the multiple-candidate initialisation of Section~\ref{sec:method} is what holds the failure rate at
\CamBowlerRefNoVerdict{} of \GeoPerCell{}; a single-candidate initialisation lost dozens more. The practical instruction is simple and the app gives it: film from behind
the batter.
...
Table~\ref{tab:kcurve} traces what $k$ buys. With the Laws' fixed band alone
...
returns no verdict rather than one it cannot defend. Real footage carries the errors the synthetic study does not model, rolling-shutter distortion, a
...
not; the real clips show only self-consistency. The repeatability result makes the next step necessary: real
deliveries with instrumented truth, most simply a second synchronised phone at right angles, so the
covariance can be checked in the field.
...
linearised covariance about one mode does not know about the other. A sampling-based uncertainty or an
explicit test of the alternative split would address this.

\textbf{The estimator ignores drag, swing and spin.} The constant-velocity model absorbs them over the short
arcs it fits, at a cost of \PhysOffAgree\% to \PhysOnAgree\% agreement, but a long pre-contact arc under
strong swing would be fitted worse, and the release speed reported is an average over the observed arc, not
the speed at the hand.

\textbf{The contact must be seen.} A full toss, a clip cut before the pitch, or a camera position from which
the batter hides the contact falls back to the single-parabola estimator, whose accuracy is the orange curve
in Fig.~\ref{fig:sweeps}, and whose result is flagged as such.
...
\EteAgree{} of \EteN{} with no false out. The propagated uncertainty is slightly optimistic and is reported as
such, and it is used as the Laws' umpire's call is used: to decline a call the evidence does not support. The
system is a coaching instrument that says how sure it is, not a substitute for a calibrated multi-camera rig,
and the paper has tried to state where the gap lies.
