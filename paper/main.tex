...
strong when the parabola is seen side-on and weak when the ball travels along the optical axis, which is the
natural phone position behind either set of stumps. The apparent size of the ball~\cite{vanzandycke2022ball3d}
...
ground plane is metres, so three further candidates are tried, the ground crossings of six-parameter parabola
fits to each side and the back-projections of the two frames adjacent to the split, and the candidate whose
...
(\TapEightAgree\% at 8~px, \TapTwelveAgree\% at 12~px), because twelve marks set the pose and the stump
plane, not any single tap, sets the anchor. Dropped frames to 40\% cost nothing measurable
...
The widening $k$ buys fewer wrong calls at the price of more declined ones. With the Laws' fixed band
alone ($k = 0$) the system returns \KZeroFalseOut{} false outs and \KZeroMissedOut{} missed outs over
\GeoTotal{} deliveries and umpire's call on \KZeroUmp\%; at $k = 0.5$, \KHalfFalseOut{} false outs and
\KHalfUmp\%; at $k = 1$, \KOneFalseOut{} and \KOneUmp\%; at $k = 1.5$, \KOneHalfFalseOut{} and \KOneHalfUmp\%;
at $k = 2$, \KTwoFalseOut{} false outs and \KTwoUmp\% umpire's calls, with missed outs falling from
\KZeroMissedOut{} to \KTwoMissedOut{} over the same range. The curve is the operating choice a deployment makes. For coaching, where a wrong decisive call
...
\GeoAgreeAllParabola\% and errors ten times larger without the anchor; through the full pipeline it agrees on
\EteAgree{} of \EteN{} with no false out. The propagated uncertainty is slightly optimistic, is reported as such,
