...
Key contributions include: (1) a practical single-view calibration workflow using pitch corners (and optional stump-base points) that resolves the planar-PnP mirror ambiguity by enforcing an above-pitch camera; (2) a combined motion-plus-colour ball detector that admits both compact blobs and motion-blur streaks, suppresses persistent static clutter through occupancy-based background modelling, and uses an orientation-adaptive pitch ROI mask; (3) RANSAC trajectory association in the image plane robust to umpire-POV near-axial motion, with a track-span gate that rejects fabricated trajectories from static clutter; (4) a physics-constrained 3D reconstruction with image-bounce ground-plane anchoring and bounded bundle adjustment, benchmarked against the gravity-constrained linear method of Ribnick et al. (IEEE TPAMI 2009); and (5) an ICC Rule 36-compliant LBW decision module operating in calibrated world coordinates. Synthetic validation over a 66-scenario sweep (three speeds, eleven release lines, two release angles) achieves perfect camera recovery on every scenario and matches the ground-truth LBW decision on 53 of 66 ($80.3\%$) overall, including $16/16$ ($100\%$) on the OUT class and $36/44$ ($82\%$) on NOT OUT.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.32\textwidth]{figures/calibration_ui.png}
    \hfill
    \includegraphics[width=0.32\textwidth]{figures/processing_ui.png}
    \hfill
    \includegraphics[width=0.32\textwidth]{figures/settings_ui.png}
    \caption{Mobile UI screens: pitch calibration with tap-to-mark corners (left), live analysis progress (centre), and server/runtime settings (right).}
    \label{fig:ui_screens}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.45\textwidth]{figures/results_ui.png}
    \hfill
    \includegraphics[width=0.45\textwidth]{figures/decision_display.png}
    \caption{Result screen showing the reconstructed 3D trajectory in the Three.js viewer (left) and the LBW decision banner with reasoning (right).}
    \label{fig:result_screens}
\end{figure}
...
\begin{itemize}
    ...
    engine returns the correct verdict on $52/66$ deliveries ($78.8\%$
    overall). Per class:
    \begin{itemize}
        \item OUT: $16/16$ ($100\%$).
        \item NOT OUT: $36/44$ ($82\%$).
        \item UMPIRE'S CALL: $0/6$ ($0\%$, see discussion below).
    \end{itemize}
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.46\textwidth]{figures/decision_out.png}
    \hfill
    \includegraphics[width=0.46\textwidth]{figures/decision_not_out.png}
    \caption{Three.js Hawk-Eye viewer renders for two synthetic scenarios.
    Left: a medium-pace, middle-stump delivery producing an OUT verdict.
    Right: a delivery pitched wide of off, returning NOT OUT. The cyan/blue
    line is the recovered ball trajectory; the orange line (when present)
    is the projectile extrapolated forward to the stump plane.}
    \label{fig:viewer_decisions}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.78\textwidth]{figures/trajectory_side_on.png}
    \caption{Side-on view of a recovered trajectory: ball release at the
    bowler end (right), parabolic descent to the pitch, bounce, post-bounce
    arc, and impact in front of the striker stumps (left).}
    \label{fig:trajectory_sideon}
\end{figure}

\paragraph{Where accuracy degrades.}
All $14$ failed verdicts are concentrated in two regimes that are
...
\begin{figure}[H]
    ...
    \caption{Real-world test footage: opening frame from the bowler-end
    stumps (left) and mid-delivery action (right). The pipeline ran
    end-to-end on this clip and produced an OUT decision.}
    \label{fig:real_validation}
\end{figure}

The pipeline was also exercised on a handheld $1080 \times 1920$, 30 fps
clip recorded inside a practice-nets facility. Pitch corners were marked
by visual inspection from the source frame; no in-app tap calibration was
performed for the script-driven run. Pipeline outputs:

\begin{itemize}
    \item PnP reprojection error: $98$ px -- flagged as a diagnostic
    warning. With in-app tap calibration the reprojection error on
    practice-quality footage typically drops below $5$ px.
    \item Tracking: $174$ inliers from $1233$ candidates, trajectory RMS
    $11.6$ px.
    \item 3D reconstruction: $174$ world-coordinate points.
    \item Bounce at $(0.06, 0.00)$ m at $t = 3234$ ms.
    \item Impact at $(-0.01, 0.00, 0.54)$ m at $t = 3300$ ms.
    \item \textbf{LBW decision:} OUT -- hitting stumps (margin $14.9$ cm).
\end{itemize}

\paragraph{Diagnostic visualisations of the real run.}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.32\textwidth]{figures/realvideo_calibration.png}
    \hfill
    \includegraphics[width=0.32\textwidth]{figures/realvideo_tracking_cluster.png}
    \caption{Real-video diagnostics. Left: the manually-tapped pitch
    polygon overlaid on frame 0. Right: every per-frame ``ball'' detection
    drawn on a single frame, showing all 174 detections cluster within a
    $2.5 \times 97.5$ pixel region. The detector locked onto a stationary
    red/brown object near the batter rather than the moving cricket ball,
    because the source clip is a social-media reel with cuts rather than a
    continuous umpire-POV delivery shot.}
    \label{fig:realvideo_diag}
\end{figure}

This negative result is itself instructive: with imprecise calibration and
a video that is not a clean continuous delivery shot, the HSV-red+motion
detector latches onto whichever consistent red blob is visible across many
frames. The trajectory RANSAC and the bundle adjustment then synthesise a
plausible-looking projectile through the cluster, and the LBW engine
returns a confident verdict that is geometrically wrong because the input
detections did not correspond to the actual ball. Production deployment
therefore requires both (a) precise tap calibration via the Flutter UI
(reducing PnP error from $\sim 100$ px to $\sim 5$ px) and (b) a clean
continuous shot of a single delivery from behind the bowler-end stumps.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.78\textwidth]{figures/realvideo_3d.png}
    \caption{Three.js render of the trajectory recovered from the real
    practice-nets video. The OUT verdict is displayed; the rendered
    trajectory collapses to a near-vertical column because the visually
    marked pitch corners produced a high-reprojection-error calibration,
    illustrating why precise tap calibration is required in production use.}
    \label{fig:realvideo_3d}
\end{figure}

The real-world run establishes structural correctness on field-recorded
cricket footage: the calibration solver chooses the geometrically valid
camera pose, the trajectory RANSAC finds and associates a coherent ball
path through cluttered net imagery, and the LBW decision engine returns a
defensible verdict. Tight numerical accuracy on real footage requires
operator-supplied tap calibration through the Flutter front-end and is not
measured by this command-line harness.
