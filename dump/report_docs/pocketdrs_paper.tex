...
post-bounce trajectory is forward-integrated to the stump plane and
adjudicated under Law~36 (leg before wicket) of the MCC Laws of Cricket, as
adopted in the International Cricket Council (ICC) playing conditions, with
handedness-aware off/leg discrimination and umpire's-call bands widened along the depth axis to reflect
monocular uncertainty. On 100 physics-based synthetic deliveries with known ground
truth, spanning five release speeds, five lines and four lengths, PocketDRS
agrees with the ground-truth verdict in 64 cases and returns no verdict in 10.
Agreement is strongly structured: 76\,\% at a good length against 48\,\% at a
yorker, and 80\,\% on middle stump against 45\,\% on the marginal $\pm$20\,cm
lines. The predicted stump-plane position is out by $35.4$\,cm on average, with
the lateral component ($9.3$\,cm) far better resolved than the vertical
($32.4$\,cm); release speed and bounce location are not recovered to useful
accuracy and are withdrawn rather than qualified. Seven not-out deliveries were
returned as out, which is the error direction that matters most. We validate end-to-end tracking on hand-held net clips,
...
\item \textbf{An MCC Law~36 decision layer with monocular uncertainty bands}:
handedness-aware and camera-end-aware off/leg discrimination and per-axis umpire's-
...
\node[dec, right=of pred] (lbw) {MCC Law\,36\\decision};
...
forward-integrated to the stump plane and adjudicated under MCC Law~36.}
\label{fig:pipeline}
\end{figure*}
...
The decision implements the three MCC Law~36 tests~\cite{mcc2017laws}:
...
\begin{algorithm}[!t]
\caption{MCC Law~36 decision with monocular bands}
\label{alg:lbw}
\begin{algorithmic}[1]
...
\paragraph*{Synthetic harness} We render 100 deliveries with known physics: five
release speeds from $90$ to $145$\,km/h, five lines (middle, and off and leg at
$20$ and $40$\,cm), and four lengths from a $2$\,m yorker to an $8$\,m short
ball, at $60$\,frames/s with the virtual camera behind the striker. Because the
generating trajectory is known exactly we measure absolute errors in the
reconstructed bounce point, impact point, predicted stump-plane crossing and
release speed, and compare the predicted verdict against a ground-truth verdict
computed from that trajectory under the same geometry the decision layer uses.
The grid is 82 not out and 18 out, so a policy answering a single class cannot
score well.
...
\begin{table}[!t]
\caption{Reconstruction error over the 100-delivery grid. Means are over the deliveries that
produced the quantity: 90 for the stump-plane and speed estimates, 73 for the bounce point.}
\label{tab:wide_err}
\centering
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Quantity} & \textbf{Mean} & \textbf{Max}\\
\midrule
Predicted stump-plane error        & $35.4$\,cm  & $80.1$\,cm\\
\quad lateral ($y$) component      & $9.3$\,cm   & $41.6$\,cm\\
\quad vertical ($z$) component     & $32.4$\,cm  & $79.1$\,cm\\
Impact-point error                 & $88.4$\,cm  & $199.6$\,cm\\
Bounce-point error                 & $404.9$\,cm & $880.1$\,cm\\
Release-speed error                & $74.6$\,km/h & $140.4$\,km/h\\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:wide_err} gives the underlying reconstruction error, and it is considerably worse
than the eight-delivery figure suggested. Across the grid the predicted stump-plane position is out
by $35.4$\,cm on average against $11.1$\,cm on the narrow set, and the intermediate quantities
degrade further: the bounce point by more than four metres and the release speed by $74.6$\,km/h,
which exceeds the range of speeds tested. Absolute speed and bounce location are not measurements
at this accuracy and we withdraw them as reported quantities rather than qualifying them.

The verdict nonetheless agrees more often than these magnitudes imply, because an LBW decision is a
thresholded question and the lateral component, at $9.3$\,cm, is the better-resolved axis. That is
the same anisotropy the narrow set showed, but the margin it leaves is much thinner than eight
favourable deliveries indicated.
...
\begin{figure}[!t]
\centering
\IfFileExists{../validation/test3/test3_sample.png}{%
  \begin{subfigure}{\columnwidth}
    \centering
    \includegraphics[width=\columnwidth,height=0.42\columnwidth,keepaspectratio]%
      {../validation/test3/test3_sample.png}
    \caption{Broadcast-style tracking overlay on a real clip.}
  \end{subfigure}\\[4pt]
  \IfFileExists{../validation/test3/test3_3d_app.png}{%
    \begin{subfigure}{\columnwidth}
      \centering
      \includegraphics[width=\columnwidth,height=0.42\columnwidth,keepaspectratio]%
        {../validation/test3/test3_3d_app.png}
      \caption{Hawk-Eye-style 3-D review view of the reconstructed trajectory
      and the predicted path to the stump plane.}
    \end{subfigure}}{}%
}{%
...
\caption{Qualitative output on the outdoor white-ball off-spin clip (test3). The
overlay shows the tracked 2-D arc over the calibrated pitch corridor and both
wickets; the 3-D Hawk-Eye view shows the reconstructed metric trajectory
forward-integrated to the stump plane, which here misses the stumps, giving
\textsc{not out}.}
\label{fig:overlay}
\end{figure}
...
\paragraph*{Absolute speed and bounce location are not recovered} A mean
release-speed error of $74.6$\,km/h over the grid exceeds the range of speeds
tested, and the bounce point is out by more than four metres on average. Speed
and down-pitch position are the coordinates a near-axial monocular view
constrains least, and at these magnitudes they are not measurements. We withdraw
them rather than report them as indicative; the system should not display either.
...
MCC Law~36 with anisotropic umpire's-call bands. On synthetic ground truth it
agrees with the true verdict in 64 of 100 deliveries and predicts the
stump-plane crossing to $35.4$\,cm on average, while being candidly weak on
