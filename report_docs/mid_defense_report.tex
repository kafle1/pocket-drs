...
\draw[flow] (mobile.south) -- node[right, font=\scriptsize] {video + calibration config} (api.north);
\draw[flow] (api.south) -- node[right, font=\scriptsize] {decoded frames for processing} (pipeline.north);
\draw[flow] (pipeline.south) -- node[right, font=\scriptsize] {track path, events, \& LBW verdict} (result.north);
\end{tikzpicture}
}
...
\draw[arrow] (s2.south) -- node[right, font=\scriptsize] {Yes} (s3.north);
